%% Анализ данных Polytech-Universe: набор 201_ori
% Русское название: Анализ исходного набора 201 с фильтрацией выбросов.
% Скрипт выполняет обработку данных магнитометра и навигации.
% Исходные файлы не изменяются.
%
% Входные файлы:
%   201_magn_ori.txt - время, Bx, By, Bz; время рассматривается как Unix time в мс.
%   201_lla_ori.csv  - RTC, RTC ms, высота, широта, долгота.
%
% В репозитории исходные файлы следует поместить в каталог:
%   Исходные данные
%
% Основные выходные файлы сохраняются в каталогах:
%   Задание 1 · Анализ данных/figures
%   Задание 1 · Анализ данных/tables
%   Задание 1 · Анализ данных/summaries/analysis_summary_201_ori.txt

clear; clc; close all;

%% Определение рабочих каталогов
% На этом этапе задаются корень репозитория, каталог исходных данных и
% папки для графиков, таблиц и текстовых сводок.
...
taskDir = fullfile(repoDir, 'Исходные данные');
resultDir = fullfile(repoDir, 'Задание 1 · Анализ данных');
...
magFile = fullfile(taskDir, '201_magn_ori.txt');
navFile = fullfile(taskDir, '201_lla_ori.csv');
...
% достаточно поместить исходные файлы в каталог "Исходные данные" внутри репозитория.
if ~isfile(magFile) || ~isfile(navFile)
    ...
    for legacyIndex = 1:numel(legacyWorkDirs)
        ...
        if strlength(legacyTaskDir) > 0
            legacyMagFile = fullfile(legacyTaskDir, '201_magn_ori.txt');
            legacyNavFile = fullfile(legacyTaskDir, '201_lla_ori.csv');
...
if ~isfile(magFile) || ~isfile(navFile)
    error(['Не найдены входные файлы 201_magn_ori.txt и 201_lla_ori.csv. ', ...
        'Поместите их в каталог "Исходные данные" внутри репозитория.']);
end

fprintf('Чтение файла магнитометра: %s\n', magFile);
magRaw = readmatrix(magFile, 'FileType', 'text');

%% Очистка и первичная статистика магнитометрии
% Скрипт восстанавливает отдельные временные метки, удаляет полностью
% недействительные строки и рассчитывает статистики по осям и модулю |B|.
...
%% Ограничение магнитометрии временным диапазоном навигации
% Для набора 201_ori дополнительно отбрасываются магнитометрические строки,
% которые лежат далеко за пределами интервала навигационных данных.
navigationTimeMarginMs = 60000;
insideNavigationRange = timeMs >= min(navTimeMs) - navigationTimeMarginMs & ...
    timeMs <= max(navTimeMs) + navigationTimeMarginMs;
cleaningCounters.excludedByNavigationTimeRangeRows = sum(~insideNavigationRange);

if cleaningCounters.excludedByNavigationTimeRangeRows > 0
    fprintf('Предупреждение: исключены строки магнитометра с временем вне диапазона навигации: %d\n', ...
        cleaningCounters.excludedByNavigationTimeRangeRows);
    mag = mag(insideNavigationRange, :);
    timeMs = mag(:, 1);
    Bx = mag(:, 2);
    By = mag(:, 3);
    Bz = mag(:, 4);
    Bnorm = sqrt(Bx.^2 + By.^2 + Bz.^2);
    magTime = datetime(timeMs / 1000, 'ConvertFrom', 'posixtime', 'TimeZone', 'UTC');
    cleaningCounters.analysisRows = size(mag, 1);
    ...
    cleaningCounters.excludedByNavigationTimeRangeRows = 0;
end
...
cleaningStats = table( ...
    ...
    'строки_исключенные_из_за_времени_вне_диапазона_навигации', ...
...
filterThresholdMGs = 750;
filteredMask = Bnorm <= filterThresholdMGs;
% Дополнительная версия результатов 201_ori_filtered строится без
% экстремальных одиночных значений выше заданного порога.
filterStats = table( ...
    numel(Bnorm), ...
    sum(~filteredMask), ...
    sum(filteredMask), ...
    filterThresholdMGs, ...
    sum(~filteredMask) / numel(Bnorm), ...
    'VariableNames', {'точек_до_фильтрации', ...
    'удалено_точек_выше_порога', ...
    'точек_после_фильтрации', ...
    'порог_фильтрации_мГс', ...
    'доля_удаленных_точек'});

magTimeFiltered = magTime(filteredMask);
BxFiltered = Bx(filteredMask);
ByFiltered = By(filteredMask);
BzFiltered = Bz(filteredMask);
BnormFiltered = Bnorm(filteredMask);
BnormStatsFiltered = makeNormStats(BnormFiltered);
spikeStatsFiltered = makeDiffSpikeStats(timeMs(filteredMask), BxFiltered, ByFiltered, BzFiltered, BnormFiltered);

writetable(cleaningStats, fullfile(tableDir, 'cleaning_stats_201_ori.csv'));
writetable(timeStats, fullfile(tableDir, 'time_stats_201_ori.csv'));
writetable(axisStats, fullfile(tableDir, 'axis_quality_201_ori.csv'));
writetable(BnormStats, fullfile(tableDir, 'magnetic_norm_stats_201_ori.csv'));
writetable(spikeStats, fullfile(tableDir, 'diff_spike_stats_201_ori.csv'));
writetable(syncStats, fullfile(tableDir, 'sync_stats_201_ori.csv'));
writetable(orbitStats, fullfile(tableDir, 'orbit_period_201_ori.csv'));
writetable(BnormStatsFiltered, fullfile(tableDir, 'magnetic_norm_stats_201_ori_filtered.csv'));
writetable(spikeStatsFiltered, fullfile(tableDir, 'diff_spike_stats_201_ori_filtered.csv'));
writetable(filterStats, fullfile(tableDir, 'filter_stats_201_ori.csv'));

fprintf('Построение графиков для отчета...\n');
plotMagneticNorm(magTime, Bnorm, spikeStats, figDir, '201_ori', '201\_ori');
plotSmoothedMagneticNorm(magTime, Bnorm, mostCommonDtMs, figDir, '201_ori', '201\_ori');
plotMagneticNorm(magTimeFiltered, BnormFiltered, spikeStatsFiltered, figDir, '201_ori_filtered', '201\_ori\_filtered');
plotSmoothedMagneticNorm(magTimeFiltered, BnormFiltered, mostCommonDtMs, figDir, '201_ori_filtered', '201\_ori\_filtered');
plotGroundTrack(nav.lonDeg, nav.latDeg, nav.altM, figDir);

fprintf('Запись текстового резюме...\n');
summaryFile = fullfile(summaryDir, 'analysis_summary_201_ori.txt');
writeSummary(summaryFile, cleaningStats, timeStats, BnormStats, syncStats, orbitStats, ...
    latPeakTimesMs, latPeakValues, filterStats, BnormStatsFiltered);
...
function plotMagneticNorm(magTime, Bnorm, spikeStats, figDir, datasetSuffix, titleSuffix)
    ...
    xlabel('Время, UTC');
    ylabel('|B|, мГс');
    title(['Модуль магнитного поля, ', titleSuffix]);
    ...
    exportgraphics(fig, fullfile(figDir, ['magnetic_field_norm_', datasetSuffix, '.png']), 'Resolution', 300);
    savefig(fig, fullfile(figDir, ['magnetic_field_norm_', datasetSuffix, '.fig']));
end

function plotSmoothedMagneticNorm(magTime, Bnorm, mostCommonDtMs, figDir, datasetSuffix, titleSuffix)
    ...
    xlabel('Время, UTC');
    ylabel('|B|, мГс');
    title(['Сглаженный тренд модуля магнитного поля, ', titleSuffix]);
    ...
    exportgraphics(fig, fullfile(figDir, ['magnetic_field_norm_smoothed_', datasetSuffix, '.png']), 'Resolution', 300);
end

function plotGroundTrack(lonDeg, latDeg, altM, figDir)
    fig = plotUnifiedGroundTrackMap(lonDeg, latDeg, altM / 1000, ...
        'Наземный трек спутника с цветовым отображением высоты, 201_lla_ori', ...
        'Высота, км');
    exportgraphics(fig, fullfile(figDir, 'ground_track_altitude_201_ori.png'), 'Resolution', 300);
    savefig(fig, fullfile(figDir, 'ground_track_altitude_201_ori.fig'));
end

function writeSummary(summaryFile, cleaningStats, timeStats, BnormStats, syncStats, orbitStats, latPeakTimesMs, latPeakValues, filterStats, BnormStatsFiltered)
    ...
    fprintf(fid, 'Сводка анализа данных Polytech-Universe, набор 201_ori\n');
    fprintf(fid, '=====================================================\n\n');
    ...
    fprintf(fid, 'Фильтрация экстремальных выбросов магнитного поля\n');
    fprintf(fid, '- Фильтр применяется только к дополнительной версии результатов `201_ori_filtered`.\n');
    fprintf(fid, '- Исходные результаты `201_ori` сохраняются без удаления магнитных выбросов.\n');
    fprintf(fid, '- Порог фильтрации: %.3f мГс.\n', tableValue(filterStats, 'порог_фильтрации_мГс'));
    fprintf(fid, '- Точек до фильтрации: %d\n', tableValue(filterStats, 'точек_до_фильтрации'));
    fprintf(fid, '- Удалено точек выше порога: %d\n', tableValue(filterStats, 'удалено_точек_выше_порога'));
    fprintf(fid, '- Точек после фильтрации: %d\n', tableValue(filterStats, 'точек_после_фильтрации'));
    fprintf(fid, '- Доля удаленных точек: %.8f\n', tableValue(filterStats, 'доля_удаленных_точек'));
    fprintf(fid, '- Максимум |B| после фильтрации: %.6f мГс\n\n', tableValue(BnormStatsFiltered, 'максимум'));
