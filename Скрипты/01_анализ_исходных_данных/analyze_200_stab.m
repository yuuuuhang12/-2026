...
% Русское название: Анализ стабилизированного набора 200.
...
%   Задание 1 · Анализ данных/summaries/analysis_summary_200_stab.txt
...
%% Очистка и подготовка магнитометрических данных
% Удаляются недействительные строки, восстанавливаются отдельные нулевые
% временные метки и рассчитывается модуль магнитного поля |B|.
...
% Для каждой навигационной точки подбирается ближайшее магнитометрическое
% измерение по времени; ошибка синхронизации сохраняется в статистике.
...
%% Формирование таблиц, графиков и текстовой сводки
% Итоговые CSV, PNG/FIG-графики и краткое резюме записываются в каталог
...
function plotGroundTrack(lonDeg, latDeg, altM, figDir)
    fig = plotUnifiedGroundTrackMap(lonDeg, latDeg, altM / 1000, ...
        'Наземный трек спутника с цветовым отображением высоты, 200_lla_stab', ...
        'Высота, км');
